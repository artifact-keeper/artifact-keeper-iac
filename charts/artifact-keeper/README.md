...
| cloud | object | `{"database":{"bootstrap":{"enabled":true},"bootstrapSecret":"","existingSecret":"","passwordKey":"password"},"features":{"plugins":false},"host":"","ingressNamespace":"ingress-nginx","plan":"","preset":"small","shared":{"databaseNamespace":"cnpg-system","scannerNamespace":"trivy","searchNamespace":"search"},"slug":"","status":"active","storage":{"accessKeyIdKey":"S3_ACCESS_KEY_ID","existingSecret":"","secretAccessKeyKey":"S3_SECRET_ACCESS_KEY"}}` | Cloud tenant overlay (instance-per-tenant SaaS) All cloud behavior is gated on cloud.slug being set. Left empty here so the default (self-hosted) profile is completely unaffected: no cloud templates render and backend/web sizing and replica counts come from the per-component values above. The values-cloud.yaml overlay and the tenants ApplicationSet supply real values for these keys. See values-cloud.yaml for the full cloud configuration and the shared-service wiring. |
| cloud.database | object | `{"bootstrap":{"enabled":true},"bootstrapSecret":"","existingSecret":"","passwordKey":"password"}` | Shared CloudNativePG database. The Postgres cluster is external to the release; the chart derives the per-tenant role/database name from the slug and reads the runtime connection string from a per-tenant Secret. |
| cloud.database.bootstrap | object | `{"enabled":true}` | Run the pre-install/pre-upgrade Job that creates the tenant role and database on the shared cluster if absent. The backend runs its own schema migrations on startup. |
| cloud.database.bootstrapSecret | string | `""` | Name of an existing Secret with superuser credentials for the shared CNPG cluster, used only by the pre-install bootstrap Job to create the tenant role and database. Expected keys: username, password. |
| cloud.database.existingSecret | string | `""` | Name of an existing Secret with the tenant DATABASE_URL (consumed by the backend) and a plain password used by the bootstrap Job to set the role password. externalDatabase.existingSecret should point at the same Secret in values-cloud.yaml. |
| cloud.database.passwordKey | string | `"password"` | Key in existingSecret holding the tenant role password (bootstrap Job). |
| cloud.features.plugins | bool | `false` | Enable WASM format plugins for this tenant. |
| cloud.host | string | `""` | Public hostname for this tenant (e.g. <slug>.artifactkeeper.cloud). Used as the single ingress host. TLS is terminated by the fleet wildcard cert, so no per-release certificate is provisioned. |
| cloud.ingressNamespace | string | `"ingress-nginx"` | ingress-nginx controller namespace, used by the per-tenant NetworkPolicy to scope ingress. |
| cloud.plan | string | `""` | Billing plan label. Informational; surfaced as a label by the ApplicationSet. |
| cloud.preset | string | `"small"` | Sizing preset: small | medium | large. Maps to chart-owned backend/web resource requests/limits and replica counts (see _helpers.tpl). |
| cloud.shared | object | `{"databaseNamespace":"cnpg-system","scannerNamespace":"trivy","searchNamespace":"search"}` | Shared in-cluster service namespaces, used by the per-tenant NetworkPolicy to scope egress. These are the namespaces of the shared CNPG cluster, search, and Trivy that every tenant reaches. |
| cloud.slug | string | `""` | Tenant slug. Empty means "not a cloud tenant" and disables every cloud code path in the chart. When set, it drives the namespace (t-<slug>), the storage prefix, and the derived Postgres role/database name (t_<slug>). |
| cloud.status | string | `"active"` | Lifecycle status: active | suspended | pending-delete. When suspended, backend and web scale to zero replicas. |
| cloud.storage | object | `{"accessKeyIdKey":"S3_ACCESS_KEY_ID","existingSecret":"","secretAccessKeyKey":"S3_SECRET_ACCESS_KEY"}` | Object storage (Cloudflare R2 via the backend S3 backend). Non-secret settings live in values-cloud.yaml (backend.env); credentials are read from a per-tenant Kubernetes Secret referenced here. |
| cloud.storage.existingSecret | string | `""` | Name of an existing Secret holding R2 credentials. Keys default to S3_ACCESS_KEY_ID and S3_SECRET_ACCESS_KEY (override below). |
...
| secrets | object | `{"jwtSecret":"","migrationEncryptionKey":"","s3AccessKey":"","s3SecretKey":"","smtpPassword":"","webhookSecretKey":""}` | Secrets These are development defaults. For production, override via --set or use existingSecret references. Never commit real credentials here. |
