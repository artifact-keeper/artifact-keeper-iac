...
| dependencyTrack | object | `{"adminPassword":"","affinity":{},"bootstrap":{"containerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]}},"enabled":false,"image":{"pullPolicy":"IfNotPresent","repository":"badouralix/curl-jq","tag":"latest@sha256:1e7c0284e24572ace7170df9fc91f15fd3b79ebf056d4dde17244d5d74bbfabc"},"podSecurityContext":{"fsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}},"containerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"readOnlyRootFilesystem":true},"database":{"existingSecret":"","secretKeys":{"host":"host","jdbcUri":"jdbc-uri","password":"password","port":"port","username":"username"}},"enabled":true,"image":{"repository":"dependencytrack/apiserver","tag":"4.11.4"},"initContainerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"readOnlyRootFilesystem":true},"nodeSelector":{},"persistence":{"size":"5Gi","storageClass":""},"podSecurityContext":{"fsGroup":1000,"runAsNonRoot":true,"runAsUser":1000},"resources":{"limits":{"cpu":"2","ephemeral-storage":"1Gi","memory":"6Gi"},"requests":{"cpu":"500m","ephemeral-storage":"256Mi","memory":"4Gi"}},"tmpSizeLimit":"2Gi","tolerations":[],"topologySpreadConstraints":[]}` | DependencyTrack SBOM analysis Provides SBOM ingestion, license analysis, and vulnerability correlation. Requires significant memory (4Gi+) to load its internal vulnerability database on startup. The bootstrap init container creates the initial admin user and API key for backend integration. |
| dependencyTrack.bootstrap.enabled | bool | `false` | Run the one-shot DependencyTrack bootstrap Job. Disabled by default because the Job requires a non-empty `dependencyTrack.adminPassword` (enforced by validateSecrets); enable it together with that password.  UPGRADE NOTE: charts <= 1.7.x defaulted this to true. If your install relied on the bootstrap Job to provision the DependencyTrack admin password and Automation API key, set enabled=true and provide adminPassword explicitly when upgrading, or the Job will no longer run. |
| dependencyTrack.bootstrap.image | object | `{"pullPolicy":"IfNotPresent","repository":"badouralix/curl-jq","tag":"latest@sha256:1e7c0284e24572ace7170df9fc91f15fd3b79ebf056d4dde17244d5d74bbfabc"}` | Image for the one-shot DependencyTrack bootstrap Job. It MUST already contain `curl` and `jq`. The bootstrap pod runs non-root (see podSecurityContext below), so installing packages at runtime with `apk add` fails ("Unable to lock database: Permission denied"); shipping the tools in the image avoids any runtime package install or network egress. The upstream image publishes only rolling tags, so the default pins by digest to keep pre-pull lists and supply-chain review stable; mirror it into your own registry for production. |
| dependencyTrack.database | object | `{"existingSecret":"","secretKeys":{"host":"host","jdbcUri":"jdbc-uri","password":"password","port":"port","username":"username"}}` | Dedicated DependencyTrack database via an existing Secret (e.g. the CloudNativePG `<cluster>-app` Secret). Only used when postgres.enabled=false AND externalDatabase.enabled=false. username/password are passed separately from jdbcUri because DependencyTrack expects them as distinct variables. |
...
| externalDatabase | object | `{"database":"artifact_registry","enabled":true,"existingSecret":"","existingSecretKey":"DATABASE_URL","host":"","password":"","port":5432,"username":""}` | External database (the default when postgres.enabled=false). Set enabled to false to switch the backend and DependencyTrack to their per-service database.existingSecret references instead (see backend.database / dependencyTrack.database and values-cnpg.yaml). |
...
| ingress | object | `{"annotations":{"nginx.ingress.kubernetes.io/enable-cors":"true"},"className":"nginx","enabled":true,"host":"artifacts.example.com","maxBodySize":"1024m","proxyReadTimeoutSeconds":300,"proxySendTimeoutSeconds":300,"tls":{"enabled":true,"secretName":"artifact-keeper-tls"}}` | Ingress configuration |
| nameOverride | string | `""` |  |
