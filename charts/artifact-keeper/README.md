...
| dependencyTrack | object | `{"adminPassword":"","affinity":{},"bootstrap":{"enabled":true},"databaseName":"dependency_track","enabled":true,"image":{"repository":"dependencytrack/apiserver","tag":"4.11.4"},"nodeSelector":{},"persistence":{"size":"5Gi","storageClass":""},"resources":{"limits":{"cpu":"2","ephemeral-storage":"1Gi","memory":"6Gi"},"requests":{"cpu":"500m","ephemeral-storage":"256Mi","memory":"4Gi"}},"tolerations":[],"topologySpreadConstraints":[]}` | DependencyTrack SBOM analysis Provides SBOM ingestion, license analysis, and vulnerability correlation. Requires significant memory (4Gi+) to load its internal vulnerability database on startup. The bootstrap init container creates the initial admin user and API key for backend integration. |
| dependencyTrack.databaseName | string | `"dependency_track"` | PostgreSQL database name used by DependencyTrack. Rendered into the JDBC URL (`jdbc:postgresql://<host>:<port>/<databaseName>`) for both the in-cluster postgres and `externalDatabase` deployment paths. Defaults to "dependency_track" so existing installs see no behaviour change. Set this to share a single managed-Postgres database with the backend (e.g. one CloudSQL DB serving the whole stack). When using the in-cluster postgres with `postgres.initDb.enabled=true`, this name is also used for the `CREATE DATABASE` statement in the bootstrap SQL. |
